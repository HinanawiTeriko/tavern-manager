from __future__ import annotations

import json
from pathlib import Path

from PIL import Image, ImageDraw, ImageEnhance


ROOT = Path(__file__).resolve().parents[2]
RAW_SOURCE = ROOT / "art_sources" / "generated_raw" / "tavern_topbar" / "tavern_topbar_ui_sheet_v2.png"
SOURCE_DIR = ROOT / "assets" / "source" / "tavern" / "topbar"
RUNTIME_DIR = ROOT / "assets" / "textures" / "ui"
MANIFEST_PATH = SOURCE_DIR / "tavern_topbar_manifest.json"
CONTACT_SHEET = ROOT / "docs" / "art" / "tavern_topbar_contact_sheet.png"
SCALE = 4

ASSETS = {
    "bar_top_panel": {
        "crop_rect": [48, 188, 1730, 348],
        "native_size": [320, 12],
        "runtime_name": "bar_top_panel",
        "safe_area": [8, 1, 312, 11],
        "intended_godot_use": "Tavern TopPanelBg long pixel UI strip",
        "colors": 28,
    },
    "topbar_menu_button_normal": {
        "crop_rect": [36, 528, 300, 666],
        "native_size": [24, 12],
        "runtime_name": "topbar_menu_button_normal",
        "safe_area": [2, 1, 22, 11],
        "intended_godot_use": "Tavern TopPanel/MenuButton normal StyleBoxTexture",
        "colors": 24,
    },
    "topbar_menu_button_hover": {
        "crop_rect": [324, 526, 588, 666],
        "native_size": [24, 12],
        "runtime_name": "topbar_menu_button_hover",
        "safe_area": [2, 1, 22, 11],
        "intended_godot_use": "Tavern TopPanel/MenuButton hover StyleBoxTexture",
        "colors": 24,
    },
    "topbar_menu_button_pressed": {
        "crop_rect": [612, 528, 876, 666],
        "native_size": [24, 12],
        "runtime_name": "topbar_menu_button_pressed",
        "safe_area": [2, 1, 22, 11],
        "intended_godot_use": "Tavern TopPanel/MenuButton pressed StyleBoxTexture",
        "colors": 24,
    },
    "topbar_end_night_button_normal": {
        "crop_rect": [900, 528, 1164, 666],
        "native_size": [24, 12],
        "runtime_name": "topbar_end_night_button_normal",
        "safe_area": [2, 1, 22, 11],
        "intended_godot_use": "Tavern TopPanel/EndNightBtn normal StyleBoxTexture",
        "colors": 24,
    },
    "topbar_end_night_button_hover": {
        "crop_rect": [1190, 526, 1454, 666],
        "native_size": [24, 12],
        "runtime_name": "topbar_end_night_button_hover",
        "safe_area": [2, 1, 22, 11],
        "intended_godot_use": "Tavern TopPanel/EndNightBtn hover StyleBoxTexture",
        "colors": 24,
    },
    "topbar_end_night_button_pressed": {
        "crop_rect": [1480, 528, 1744, 666],
        "native_size": [24, 12],
        "runtime_name": "topbar_end_night_button_pressed",
        "safe_area": [2, 1, 22, 11],
        "intended_godot_use": "Tavern TopPanel/EndNightBtn pressed StyleBoxTexture",
        "colors": 24,
    },
}


def remove_chroma_key(image: Image.Image) -> Image.Image:
    rgba = image.convert("RGBA")
    pixels = rgba.load()
    for y in range(rgba.height):
        for x in range(rgba.width):
            r, g, b, a = pixels[x, y]
            if g >= 130 and g > r * 1.3 and g > b * 1.3 and g > max(r, b) + 24:
                pixels[x, y] = (0, 0, 0, 0)
    return rgba


def quantize_visible(image: Image.Image, colors: int) -> Image.Image:
    rgba = image.convert("RGBA")
    alpha = rgba.getchannel("A").point(lambda value: 255 if value >= 96 else 0)
    rgb = Image.new("RGB", rgba.size, (0, 0, 0))
    rgb.paste(rgba.convert("RGB"), mask=alpha)
    quantized = rgb.quantize(colors=colors, method=Image.Quantize.MEDIANCUT).convert("RGBA")
    quantized.putalpha(alpha)
    pixels = quantized.load()
    for y in range(quantized.height):
        for x in range(quantized.width):
            if pixels[x, y][3] == 0:
                pixels[x, y] = (0, 0, 0, 0)
    return quantized


def normalize_asset(reference: Image.Image, spec: dict[str, object]) -> Image.Image:
    native_size = tuple(spec["native_size"])
    crop = reference.crop(tuple(spec["crop_rect"]))
    keyed = remove_chroma_key(crop)
    resized = keyed.resize(native_size, Image.Resampling.LANCZOS)
    rgb = resized.convert("RGB")
    contrast = ImageEnhance.Contrast(rgb).enhance(1.18)
    color = ImageEnhance.Color(contrast).enhance(1.05)
    sharp = ImageEnhance.Sharpness(color).enhance(1.6).convert("RGBA")
    sharp.putalpha(resized.getchannel("A"))
    return quantize_visible(sharp, int(spec["colors"]))


def save_assets(reference: Image.Image) -> dict[str, Image.Image]:
    SOURCE_DIR.mkdir(parents=True, exist_ok=True)
    RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
    outputs: dict[str, Image.Image] = {}
    for asset_id, spec in ASSETS.items():
        native = normalize_asset(reference, spec)
        native_path = SOURCE_DIR / f"{asset_id}_native.png"
        runtime_path = RUNTIME_DIR / f"{spec['runtime_name']}.png"
        native.save(native_path)
        runtime = native.resize(
            (native.width * SCALE, native.height * SCALE),
            Image.Resampling.NEAREST,
        )
        runtime.save(runtime_path)
        outputs[asset_id] = native
    return outputs


def write_manifest() -> None:
    manifest_assets = {}
    for asset_id, spec in ASSETS.items():
        native_size = list(spec["native_size"])
        runtime_size = [native_size[0] * SCALE, native_size[1] * SCALE]
        manifest_assets[asset_id] = {
            "crop_rect": spec["crop_rect"],
            "native": f"assets/source/tavern/topbar/{asset_id}_native.png",
            "runtime": f"assets/textures/ui/{spec['runtime_name']}.png",
            "native_size": native_size,
            "runtime_size": runtime_size,
            "safe_area": spec["safe_area"],
            "intended_godot_use": spec["intended_godot_use"],
        }
    manifest = {
        "id": "tavern_topbar",
        "source": "art_sources/generated_raw/tavern_topbar/tavern_topbar_ui_sheet_v2.png",
        "prompt": "art_sources/generated_raw/tavern_topbar/tavern_topbar_ui_sheet_v2_prompt.txt",
        "scale": SCALE,
        "assets": manifest_assets,
    }
    MANIFEST_PATH.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def make_contact_sheet(reference: Image.Image, natives: dict[str, Image.Image]) -> None:
    CONTACT_SHEET.parent.mkdir(parents=True, exist_ok=True)
    sheet = Image.new("RGBA", (1000, 560), (18, 14, 11, 255))
    draw = ImageDraw.Draw(sheet)
    draw.text((16, 14), "Tavern topbar v2 pipeline - dedicated sheet", fill=(220, 204, 176, 255))
    draw.text((16, 42), "generated source and explicit crop boxes", fill=(220, 204, 176, 255))
    source_preview = reference.resize((960, 480), Image.Resampling.LANCZOS).convert("RGBA")
    sheet.alpha_composite(source_preview, (16, 64))
    for spec in ASSETS.values():
        crop = spec["crop_rect"]
        scale_x = 960 / reference.width
        scale_y = 480 / reference.height
        rect = (
            int(crop[0] * scale_x) + 16,
            int(crop[1] * scale_y) + 64,
            int(crop[2] * scale_x) + 16,
            int(crop[3] * scale_y) + 64,
        )
        draw.rectangle(rect, outline=(255, 188, 92, 255), width=1)

    y = 500
    topbar = natives["bar_top_panel"].resize((640, 24), Image.Resampling.NEAREST)
    sheet.alpha_composite(topbar, (16, y))
    x = 672
    for asset_id in [asset for asset in ASSETS if asset.startswith("topbar_")]:
        preview = natives[asset_id].resize((48, 24), Image.Resampling.NEAREST)
        sheet.alpha_composite(preview, (x, y))
        x += 52
    sheet.convert("RGB").save(CONTACT_SHEET)


def main() -> None:
    if not RAW_SOURCE.exists():
        raise FileNotFoundError(f"missing Tavern topbar source: {RAW_SOURCE}")
    reference = Image.open(RAW_SOURCE).convert("RGBA")
    outputs = save_assets(reference)
    write_manifest()
    make_contact_sheet(reference, outputs)
    print("exported Tavern topbar v2 assets")
    print("contact sheet: docs/art/tavern_topbar_contact_sheet.png")


if __name__ == "__main__":
    main()
