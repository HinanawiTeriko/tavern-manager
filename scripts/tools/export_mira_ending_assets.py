...
def load_native(route: str) -> Image.Image:
    path = SOURCE / f"mira_{route}_native.png"
    if not path.exists():
        raise FileNotFoundError(f"{path}: missing native Mira ending source")
...
def write_manifest() -> None:
    ...
    for route in ROUTES:
        routes[route] = {
            "reference": f"assets/source/endings/mira/reference/mira_{route}_reference_{REFERENCE_VERSION}.png",
            "native": f"assets/source/endings/mira/mira_{route}_native.png",
            "runtime": f"assets/textures/endings/mira/mira_{route}.png",
            "safe_area": [0, 0, NATIVE_SIZE[0], NATIVE_SIZE[1]],
            "intended_godot_use": "Mira fate cinematic layer before Day 12 ledger settlement",
        }
    manifest = {
        "id": "mira_ending_backgrounds",
...
def main() -> None:
    ...
    for route in ROUTES:
        reference = REFERENCE / f"mira_{route}_reference_{REFERENCE_VERSION}.png"
        if not reference.exists():
            raise FileNotFoundError(f"{reference}: missing approved Mira ending reference")
        native = load_native(route)
        runtime = export_runtime(route, native)
        runtime.save(RUNTIME / f"mira_{route}.png")
