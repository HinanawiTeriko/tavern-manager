...
MANIFEST_PATH = ROOT / "assets" / "source" / "daymap" / "markers" / "daymap_grey_ledger_marker_manifest.json"
SOURCE_IMAGE = ROOT / "art_sources" / "generated_raw" / "daymap_markers" / "grey_ledger_markers_source_v1.png"
...
def load_source_image(manifest: dict) -> Image.Image:
    ...
    if not source_path.exists():
        raise FileNotFoundError(f"Missing retained grey ledger marker source sheet: {source_path}")
...
def trim_to_alpha(image: Image.Image) -> Image.Image:
    ...
    if box is None:
        raise ValueError("grey ledger marker crop is empty after magenta-key removal")
