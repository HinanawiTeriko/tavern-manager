...
def validate_rect(rect: tuple[int, int, int, int], image: Image.Image, icon_id: str) -> None:
    ...
    if left < 0 or top < 0 or right > image.width or bottom > image.height:
        raise ValueError(f"{icon_id}: source_rect outside reference bounds")
...
def materialize_reference(icon_id: str, spec: dict) -> Image.Image:
    ...
    if "source_sheet" in spec:
        sheet = Image.open(project_path(spec["source_sheet"])).convert("RGBA")
        sheet_rect = tuple(int(v) for v in spec["sheet_rect"])
        validate_rect(sheet_rect, sheet, icon_id)
        reference_path.parent.mkdir(parents=True, exist_ok=True)
        sheet.crop(sheet_rect).save(reference_path)
...
def make_contact_sheet(exports: list[dict]) -> None:
    ...
    row_height = 136
    sheet = Image.new("RGBA", (620, 36 + row_height * len(exports)), (18, 14, 11, 255))
    draw = ImageDraw.Draw(sheet)
    headers = [("id", 12), ("reference", 96), ("manifest crop", 220), ("native 4x", 344), ("runtime", 468)]
    ...
    for row, exported in enumerate(exports):
        ...
        for preview, x in zip(previews, [96, 220, 344, 468]):
...
def main() -> None:
    ...
    print("exported tavern item icons: " + ", ".join(exported["id"] for exported in exports))
