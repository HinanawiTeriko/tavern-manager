...
def export_icon(icon_id: str, spec: dict) -> dict:
    source = Image.open(project_path(spec["source"])).convert("RGBA")
    rect = tuple(int(v) for v in spec["source_rect"])
    validate_rect(rect, source, icon_id)
    crop = source.crop(rect)
    cutout = remove_chroma(crop, int(spec.get("chroma_threshold", 120)))
...
def make_contact_sheet(exports: list[dict]) -> None:
    ...
    headers = [("id", 12), ("crop", 190), ("native 4x", 314), ("runtime", 438)]
    ...
    for row, exported in enumerate(exports):
        ...
        for preview, x in zip(previews, [190, 314, 438]):
...
def main() -> None:
    ...
    print("exported recipe icons: " + ", ".join(exported["id"] for exported in exports))
