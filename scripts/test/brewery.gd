...
## 酒桶（容器）：物品以足够速度进入 Mouth Area2D 才被接收（砸进 / 甩进皆可）。
## 力度不够（进桶瞬间 linear_velocity < MIN_THROW_SPEED）→ 不消耗，物品穿过桶口落桌上。
...
const MIN_THROW_SPEED := 250.0   # 进桶瞬间速度阈值；手感验证：砸~2300 收 / 轻挪<120 拒
...
func _on_mouth_body_entered(body: Node) -> void:
	...
	if GameManager.craft.is_product(item.item_key):
		return
	var entry_speed: float = item.linear_velocity.length()
	if entry_speed < MIN_THROW_SPEED:
		return
	# 够力 → 接收
