...
func _check_loads_expected_laws() -> void:
	...
	_ok(float(gugu_waddle.get("linear_damp_multiplier", 1.0)) < 0.2, "gugu law should make items keep sliding")
	_ok(float(gugu_waddle.get("release_spin_multiplier", 1.0)) > 1.0, "gugu law should add sleepy table spin")
	_ok(float(orunji_bounce.get("bounce_override", 0.0)) >= 0.75, "orunji law should make items doro-bouncy")
	_ok(float(orunji_bounce.get("random_lift_impulse", 0.0)) > 0.0, "orunji law should occasionally hop items")
	_ok(float(anon_hia_laugh.get("gravity_scale_multiplier", 1.0)) < 1.0, "anon hia laugh law should make the table feel light")
	_ok(float(anon_hia_laugh.get("release_impulse_multiplier", 1.0)) > 1.0, "anon hia laugh law should give floaty release drift")
	_ok(float(anon_hia_laugh.get("random_lift_impulse", 0.0)) > 0.0, "anon hia laugh law should pulse items upward")
	_ok(float(nailong_laugh.get("gravity_scale_multiplier", 1.0)) > 1.0, "nailong laugh law should make items heavy")
	_ok(float(nailong_laugh.get("near_customer_pull", 0.0)) > 0.0, "nailong laugh law should pull loose food toward the belly")
