...
**分支**: `main`
**阶段**: v0.2 — 5场景流转、7子系统、手势合成、商店、ThemeColors UI 设计系统、莱恩3天叙事线
...
| `scripts/GameManager.cs` | Autoload 顶层协调器，库存/快捷栏/配方/拖拽/对话 |
...
| `scripts/systems/ShopSystem.cs` | 商店配置、材料售价、配方解锁 |
| `scripts/systems/NarrativeManager.cs` | NPC 剧情配置、DM 驱动、变量管理（SetVar 用 Variant 非 object） |
| `scripts/ui/TavernView.cs` | 酒馆营业主界面 |
| `scripts/ui/CraftStation.cs` | 合成台手势组件（拖拽/加热/摇晃/搅拌） |
| `scripts/ui/DayMapView.cs` | 白天大地图界面（含商店标签页） |
| `scripts/ui/TitleScreen.cs` | 标题界面 |
| `scripts/ui/EndingScreen.cs` | 结局结算界面 |
| `scripts/ui/LedgerScreen.cs` | 打烊后账单界面 |
| `scripts/ui/ThemeColors.cs` | "Hearth & Shadow" 全局配色 + StyleBox 工厂 |
| `scripts/ui/TextureManager.cs` | 纹理加载缓存工具 |
...
- ❌ `Variant.From()` 禁传 `object` 类型参数 — 编译期 T 解析为 `System.Object`，Godot 不支持该转换。方法签名用 `Variant` 类型，靠 C# 隐式转换处理
...
3. **Worktree** — `git worktree add` 创建 `feature/<topic>` 隔离工作区
4. **编码** — 严格按 plan 顺序，每任务完成立即 `dotnet build`，禁止顺手重构，禁止加 plan 外功能
5. **验证** — Godot 跑起来走核心路径 + 回归检查 + `dotnet build` 0 errors
...
`serve_result` 由 GameManager 上菜逻辑设置 (`"success"` / `"fail"`)，通过 `NarrativeManager.DialogueVars` 传给 Dialogue Manager。
